...
这篇文章的目标很简单：**把最基础、最常见的 AI 词汇，用工程师能理解的方式讲透**。不堆砌定义，不制造焦虑，帮你建立一个清晰的概念坐标系。后续还会有两篇，分别覆盖模型架构与训练方法、以及工程落地与生态工具。
...
关于 LLM 的详细工作原理，我之前写过一篇[《大模型是怎么听懂人话的》](/blog/da-mo-xing-shi-zen-me-ting-dong-ren-hua-de)，这里不再展开。
...
- **LLM** 是最近最火的技术路径
- **AIGC** 是 LLM 带来的产业现象
- **Prompt** 是你和 LLM 的交互界面
- **RAG** 解决 LLM 知识不足的问题
- **Agent** 让 LLM 从"说话"进化到"做事"
- **多模态** 让 LLM 能处理图文音视频
- **涌现** 解释了大模型为什么"突然变聪明"
- **对齐** 让大模型变得安全可用
- **幻觉** 是大模型最致命的局限性

下一篇，我会深入讲解模型架构和训练相关的词汇：Transformer、Attention、Embedding、Fine-tuning、LoRA、MoE、Token、Temperature 等。
